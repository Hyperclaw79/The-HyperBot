...
from Hyperbot.playlist import Playlist
from Hyperbot.player import MusicPlayer
from Hyperbot.config import Config, ConfigDefaults
from Hyperbot.permissions import Permissions, PermissionsDefaults
from Hyperbot.utils import load_file, write_file, sane_round_int
...
class HyperBot(discord.Client):
    def __init__(self, config_file=ConfigDefaults.options_file, perms_file=PermissionsDefaults.perms_file):
        ...
        self.http.user_agent += ' HyperBot/%s' % BOTVERSION
    ...
    def run(self):
        try:
            try:
                ...
                raise exceptions.HelpfulError('Bot cannot login, bad credentials.', 'Fix your Email or Password or Token in the options file.  Remember that each field should be on their own line.')
    ...
    async def on_server_join(self, server):
        memlist = self.get_all_members()
        servc = server
        ownerc = find(lambda m: m.id == self.config.owner_id, memlist)
    ...
    async def on_ready(self):
        ...
        if not self.initgame:
            gamey = discord.Game(name='with bytes of binary data and shitting out codes.')
        else:
            gamey = self.initgame
        await self.change_presence(game=gamey)
        if self.config.HiSwitch == True:
            for server in self.servers:
                chan = self.get_channel(server.id)

            if self.Flag == False:
                await self.send_message(chan, "Hi, I'm online.")
            else:
                await self.send_message(chan, 'I am back! :metal::skin-tone-1:')
        if self.config.owner_id == self.user.id:
            raise exceptions.HelpfulError("Your OwnerID is incorrect or you've used the wrong credentials.", 'The bot needs its own account to function.  The OwnerID is the id of the owner, not the bot.  Figure out which one is which and use the correct information.')
            self.init_ok = True
        ...
        if self.config.bound_channels:
           ...
           await self.safe_print(' - %s/%s' % (ch.server.name.strip(), ch.name.strip()) for ch in chlist if ch)
           if invalids and self.config.debug_mode:
              print('\nNot binding to voice channels:')
              await self.safe_print(' - %s/%s' % (ch.server.name.strip(), ch.name.strip()) for ch in invalids if ch)
              print()
           else:
        ...
        print('  Skip threshold: %s votes or %s%%' % (self.config.skips_required, self._fixg(self.config.skip_ratio_required * 100)))
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        Adds the song to the playlist.  If a link is not provided, the first result from a youtube search is added to the queue.
        """
        song_url = song_url.strip('<>')
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError('You have reached your enqueued song limit (%s)' % permissions.max_songs, expire_in=30)
        ...
        if info.get('url', '').startswith('ytsearch'):
            info = await (self.downloader.extract_info(player.playlist.loop, song_url, download=False, process=True, on_error=lambda e: asyncio.ensure_future(self.safe_send_message(channel, '```\n%s\n```' % e, expire_in=120), loop=self.loop), retry_on_error=True))
            if not info:
                raise exceptions.CommandError('Error extracting info from search string, youtubedl returned no data.  You may need to restart the bot if this continues to happen.', expire_in=30)
        ...
        if 'entries' in info:
            ...
            if permissions.max_playlist_length and num_songs > permissions.max_playlist_length:
                raise exceptions.PermissionsError('Playlist has too many entries (%s > %s)' % (num_songs, permissions.max_playlist_length), expire_in=30)
            if permissions.max_songs and player.playlist.count_for_user(author) + num_songs > permissions.max_songs:
                raise exceptions.PermissionsError('Playlist entries + your already queued songs reached limit (%s + %s > %s)' % (
                 num_songs, player.playlist.count_for_user(author), permissions.max_songs), expire_in=30)
        if info['extractor'].lower() in ('youtube:playlist', 'soundcloud:set', 'bandcamp:album'):
            ...
            try:
                t0 = time.time()
                wait_per_song = 1.2
                procmesg = await (self.safe_send_message(channel, 'Gathering playlist information for {} songs{}'.format(num_songs, ', ETA: {} seconds'.format(self._fixg(num_songs * wait_per_song)) if num_songs >= 10 else '.')))
                await self.send_typing(channel)
            except Exception:
                traceback.print_exc()
                raise exceptions.CommandError('Error handling playlist %s queuing.' % playlist_url, expire_in=30)

        if extractor_type.lower() in ('soundcloud:set', 'bandcamp:album'):
            try:
                entries_added = await (player.playlist.async_process_sc_bc_playlist(playlist_url, channel=channel, author=author))
            except Exception:
                traceback.print_exc()
                raise exceptions.CommandError('Error handling playlist %s queuing.' % playlist_url, expire_in=30)
            ...
            ttime = tnow - t0
            wait_per_song = 1.2
            print('Processed {}/{} songs in {} seconds at {:.2f}s/song, {:+.2g}/song from expected ({}s)'.format(songs_processed, num_songs, self._fixg(ttime), ttime / num_songs, ttime / num_songs - wait_per_song, self._fixg(wait_per_song * num_songs)))
            if not songs_added:
                basetext = 'No songs were added, all songs were over max duration (%ss)' % permissions.max_song_length
                if skipped:
                    basetext += '\nAdditionally, the current song was skipped for being too long.'
                raise exceptions.CommandError(basetext, expire_in=30)
        return Response('Enqueued {} songs to be played in {} seconds'.format(songs_added, self._fixg(ttime, 1)), delete_after=30)
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        """
        if permissions.max_songs and player.playlist.count_for_user(author) > permissions.max_songs:
            raise exceptions.PermissionsError('You have reached your playlist item limit (%s)' % permissions.max_songs, expire_in=30)

        def argcheck():
            if not leftover_args:
                raise exceptions.CommandError('Please specify a search query.\n%s' % dedent(self.cmd_search.__doc__.format(command_prefix=self.config.command_prefix)), expire_in=60)
        ...
        def check(m):
            return m.content.lower()[0] in 'yn' or m.content.lower().startswith('{}{}'.format(self.config.command_prefix, 'search')) or m.content.lower().startswith('exit')
        ...
        for e in info['entries']:
            ...
            if response_message.content.startswith(self.config.command_prefix) or response_message.content.lower().startswith('exit'):
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            return Response('There are no songs queued! Queue something with {}play.'.format(self.config.command_prefix), delete_after=30)
        ...
        if not player.current_entry:
            if player.playlist.peek():
                ...
                if player.playlist.peek().is_downloaded:
                    ...
                    print("Something odd is happening.  You might want to restart the bot if it doesn't start working.")
            else:
                print("Something strange is happening.  You might want to restart the bot if it doesn't start working.")
            if author.id == self.config.owner_id or permissions.instaskip or author == player.current_entry.meta.get('author', None):
                player.skip()
                await self._manual_delete_check(message)
                return
            num_voice = sum((1 for m in voice_channel.voice_members if not (m.deaf or m.self_deaf or m.id in [self.config.owner_id, self.user.id])))
            num_skips = player.skip_state.add_skipper(author.id, message)
            skips_remaining = min(self.config.skips_required, sane_round_int(num_voice * self.config.skip_ratio_required)) - num_skips
            if skips_remaining <= 0:
                player.skip()
                return Response('your skip for **{}** was acknowledged.\nThe vote to skip has been passed.{}'.format(player.current_entry.title, ' Next song coming up!' if player.playlist.peek() else ''), reply=True, delete_after=20)
            return Response('your skip for **{}** was acknowledged.\n**{}** more {} required to vote to skip this song.'.format(player.current_entry.title, skips_remaining, 'person is' if skips_remaining == 1 else 'people are'), reply=True, delete_after=20)

    def cmd_volume(self, message, player, new_volume=None):
        """
        Usage:
        ...
        if relative:
            raise exceptions.CommandError('Unreasonable volume change provided: {}{:+} -> {}%.  Provide a change between {} and {:+}.'.format(old_volume, vol_change, old_volume + vol_change, 1 - old_volume, 100 - old_volume), expire_in=20)
        else:
            raise exceptions.CommandError('Unreasonable volume provided: {}%. Provide a value between 1 and 100.'.format(new_volume), expire_in=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            for i, item in enumerate(player.playlist, 1):
                ...
                if currentlinesum + len(nextline) + len(andmoretext) > DISCORD_MSG_CHAR_LIMIT and currentlinesum + len(andmoretext):
                    unlisted += 1
                    continue
            ...
            if unlisted:
                lines.append('\n*... and %s more*' % unlisted)
            if not lines:
                lines.append('There are no songs queued! Queue something with {}play.'.format(self.config.command_prefix))
            message = '\n'.join(lines)
    ...
    async def cmd_pldump(self, channel, song_url):
        ...
        if not info.get('entries', None):
            ...
            linegens = defaultdict((lambda : None), **{'youtube': lambda d: 'https://www.youtube.com/watch?v=%s' % d['id'],
            ...
            with BytesIO() as fcontent:
                for item in info['entries']:
                    fcontent.write(exfunc(item).encode('utf8') + '\n')
    ...
    @owner_only
    def cmd_blacklist(self, message, user_mentions, option, something):
        """
        Usage:
        ...
        if option not in ('+', '-', 'add', 'remove'):
            raise exceptions.CommandError('Invalid option "%s" specified, use +, -, add, or remove' % option, expire_in=20)
        ...
        if option in ('+', 'add'):
            ...
            return Response('%s users have been added to the blacklist' % (len(self.blacklist) - old_len), reply=True, delete_after=10)
        if self.blacklist.isdisjoint((user.id for user in user_mentions)):
            return Response('none of those users are in the blacklist.', reply=True, delete_after=10)
        self.blacklist.difference_update((user.id for user in user_mentions))
        write_file(self.config.blacklist_file, self.blacklist)
        return Response('%s users have been removed from the blacklist' % (old_len - len(self.blacklist)), reply=True, delete_after=10)
    ...
    @owner_only
    async def cmd_restart(self, channel, message):
        """
        Usage: Owner only.
        ...
        self.Flag = True
        print(self.Flag)
        subprocess.call(['runbot.bat'])

    @owner_only
    async def cmd_byeserver(self, message):
        await self.send_message(message.channel, "I'm outta this shithole! :wave:")
        await asyncio.sleep(5)
        await self.leave_server(message.server)

    @owner_only
    async def cmd_hypersama(self, message):
        servc = message.server
        rolec = await (self.create_role(servc))
        roleadmin = discord.utils.get(message.server.roles, name='Admin')
        rolebot = discord.utils.get(message.server.roles, name='HyperBot')
        roletop = message.server.me.top_role
        ...
        await self.add_roles(mem, rolehyper)
    ...
    async def cmd_setgame(self, message):
        ...
        gamey = discord.Game(name=message_content)
        await self.change_presence(game=gamey)
        self.initgame = gamey
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        def is_possible_command_invoke(entry):
            valid_call = any((entry.content.startswith(prefix) for prefix in [self.config.command_prefix]))
        ...
        if self.user.bot and channel.permissions_for(server.me).manage_messages:
        ...
        async for entry in self.logs_from(channel, search_range, before=message):
            ...
            if is_possible_command_invoke(entry) and delete_invokes and (delete_all or entry.author == author):
                try:
    ...
    async def cmd_listids(self, server, author, leftover_args, cat='all'):
        ...
        if cat == 'all':
            ...
            requested_cats = [
             cat] + [c.strip(',') for c in leftover_args]
        ...
        with BytesIO() as sdata:
            sdata.writelines((d.encode('utf8') + '\n' for d in data))
            sdata.seek(0)
    ...
    async def cmd_help(self, message, command=None):
        ...
        if command:
            ...
            if cmdc:
                return Response('```cs\n{}```'.format(dedent(cmdc.__doc__).replace('{command_prefix}', '#' + self.config.command_prefix)), delete_after=300)
            return Response('No such command', delete_after=10)
        else:
            msg1 = await (self.send_message(message.channel, '**HyperBot Commands List:**\n'))
            commands = []
            cmdc = {}
            txt1 = ''
            txt2 = ''
    ...
    async def cmd_avatar(self, message, user_mentions):
        """
        Usage:
            {command_prefix}avatar [@user or None]
        
        Display your avatar or avatar of another user.
        """
        await self.send_message(message.channel, 'Here is the avatar you blindy:\n')
        if not user_mentions:
            await self.send_message(message.channel, 'Showing off your own shit eh? Mr. %s\n%s' % (message.author.mention, message.author.avatar_url))
        else:
            user = user_mentions[0]
            await self.send_message(message.channel, "Enjoying stalking %s's avatar.\n%s" % (user.mention, user.avatar_url))

    async def cmd_joinserver(self, message, author, server_link=None):
        """
        Usage:
        ...
        if self.user.bot:
            ...
            url += '&permissions=1534590153'
            await self.send_message(author, "Bot accounts can't use invite links!  Click here to invite me: %s \n" % url)
            await self.send_message(message.server.get_member(self.config.owner_id), '\n %s asked for an invite from %s server.' % (message.author, message.server))
            return Response(':mailbox_with_mail:', delete_after=20)
        try:
            if server_link:
    ...
    async def cmd_say(self, message):
        ...
        Make the bot say your message anonymously.
        """
        message_content = message.content.strip()
        await self.delete_message(message)
    ...
    async def cmd_pissed(self, message):
        """
        Usage:
            {command_prefix}pissed
                         
        Say that you're pissed, with a dank meme.
        """
        pisem = discord.Embed(title='SO FKIN PISSED!', description=' ', color=7995392)
        pisem.set_image(url='http://pre08.deviantart.net/51d6/th/pre/i/2009/018/6/c/angry_toilet_by_iamderek.jpg')
        await self.send_message(message.channel, content=None, embed=pisem)

    async def cmd_salt(self, message, user_mentions):
        """
        Usage:
            {command_prefix}pissed[@user or None.]
        ...
        if not user_mentions:
            targem.set_image(url=message.author.avatar_url)
        else:
            user = user_mentions[0]
            targem.set_image(url=user.avatar_url)
        await self.send_message(message.channel, content=None, embed=targem)
    ...
    async def cmd_afk(self, message):
        ...
        Let others know that you are AFK, also to those who tag you.             
        """
        await self.send_message(message.channel, "```apache\n%s is now Away From their Keyboard. Don't bother interacting with them unless they talk first.```" % message.author.display_name)
        self.afk = True
        self.afkuser = message.author
        self.afkdict[message.author] = str(message.author)
    ...
    async def cmd_ping(self, message):
        ...
        """
        msgtime = time.time()
        await self.send_message(message.channel, 'Pinging....')
        now = time.time()
        ping = now - msgtime
        try:
            pong = discord.Embed(title='Pong! Response Time:', description='%0.01f s' % ping, color=7995392)
            pong.set_thumbnail(url='https://cdn0.vox-cdn.com/thumbor/q8SAVwqBqH34VlWw2lae0x_arq8=/61x0:1858x1011/1600x900/cdn0.vox-cdn.com/uploads/chorus_image/image/29836791/tabletennis-robot.0.jpg')
    ...
    async def cmd_choose(self, message):
        """
        Usage: 
        ...
        """
        await chooser.get_choose(self, message, self.config.command_prefix)

    def value_editor(self, text, index=0, modify_to=''):
        return '%s%s%s' % (text[:index], modify_to, text[index + 1:])

    @owner_only
    async def cmd_reload_race(self):
        reload(race)

    async def cmd_race(self, message, user_mentions):
        """
        Usage:
        ...
        """
        await race.play_race(self, message, user_mentions)

    @owner_only
    async def cmd_reload_profiles(self):
        reload(profiles)

    @owner_only
    async def cmd_create_profs(self, message):
        await profiles.get_create_profs(self, message)

    async def cmd_profile(self, message, author, user_mentions):
        """
        Usage:
        {command_prefix}profile @user
        
        Create your citizenship in the HyperRealm or check your growth via the HyperProfile card. Mention a user for viewing their citizenship.
        """
    ...
    async def cmd_setbio(self, message):
        """
        Usage:
        {command_prefix}setbio your_bio 
         
        Update your bio in the H!profile.
        """
        await profiles.get_setbio(self, message)

    async def cmd_marry(self, message, user_mentions):
        """
        Usage:
        ...
        """
        await profiles.get_marry(self, message, user_mentions)

    async def cmd_divorce(self, message, user_mentions):
        """
        Usage:
            {command_prefix}divorce @user 
                         
        Divorce your husbando/waifu. Psst. Sorry to hear that btw.
        """
        await profiles.get_divorce(self, message, user_mentions)

    @owner_only
    async def cmd_reload_currency(self):
        reload(currency)

    async def cmd_daily(self, message, author):
        """
        Usage:
        {command_prefix}daily
        
        Get your daily 100 Hypercoins. You can collect once in every 24hrs.
        """
        await currency.get_daily(self, message, author)

    async def cmd_checkbalance(self, message):
        """
        Usage:
        {command_prefix}checkbalance
        
        Check your account balance in the HyperBank.
        """
        await currency.get_checkbalance(self, message)

    async def cmd_transfer(self, message, amt, user_mentions):
        """
        Usage:
            {command_prefix}transfer amt @user
        
        Feeling generous? Transfer Hypercoins to another user.
        """
        await currency.get_transfer(self, message, amt, user_mentions)
		
    async def cmd_buy_horse(self, message):
        """
        Usage:
        {command_prefix}buy_horse
        
        Buy a horse with robotcoins to give you edge in H!race.
        """
        await currency.get_buy_horse(self, message)

    @owner_only
    async def cmd_reload_interactions(self):
        reload(interactions)

    async def cmd_punch(self, message, user_mentions):
        """
        Usage: 
        ...
        """
        await interactions.get_punch(self, message, user_mentions)
    ...
    async def cmd_pat(self, message, user_mentions):
        ...
        """
        await interactions.get_pat(self, message, user_mentions)
    ...
    async def cmd_hug(self, message, user_mentions):
        ...
        """
        await interactions.get_hug(self, message, user_mentions)
    ...
    async def cmd_kiss(self, message, user_mentions):
        ...
        """
        await interactions.get_kiss(self, message, user_mentions)

    async def cmd_flip(self, message, user_mentions):
        """
        Usage: 
        ...
        """
        await interactions.get_flip(self, message, user_mentions)

    @owner_only
    async def cmd_reload_games(self):
        reload(games)

    async def cmd_minesweeper(self, message):
        """
        Usage: 
        {command_prefix}minesweeper 
                          
        Play a quick game of Minesweeper.        
        """
        await games.get_minesweeper(self, message)

    @owner_only
    async def cmd_reload_rpg(self):
        reload(rpg)

    async def cmd_create_character(self, message):
        """
        Usage: 
        {command_prefix}create_character 
    ...
    async def on_message(self, message):
        ...
        if not message.author == self.user and message.author.bot != True:
            await levels.get_levels(self, message, message.author.id)
        msg_content = message.content.strip('')
        userc = discord.User()
        if '@' in msg_content:
            try:
                userc = find(lambda m: msg_content.split('@')[1].split('>')[0] in m.id, message.server.members)
                if str(userc) in self.afkdict.get(userc):
                    await self.send_message(message.channel, "```py\nHey %s, '%s' is AFK.```" % (message.author.display_name, userc.display_name))
                else:
                    pass
            except:
                pass
        ...
        if message_content.lower().startswith('hey'):
          if message.author == self.user:
             return
          searchword = message_content.split(' ')[1]
          try:
             culprit = find(lambda m: searchword.lower() in m.display_name.lower(), message.server.members)
          except:
             culprit = find(lambda m: searchword.lower() in m.name.lower(), message.server.members)

          try:
             mesrep = message_content.replace(searchword, culprit.mention)
             await self.send_message(message.channel, mesrep)
             await self.send_message(message.channel, '\n %s You can thank me later for the tag buddy. :wink:' % message.author.mention)
          except:
             pass
