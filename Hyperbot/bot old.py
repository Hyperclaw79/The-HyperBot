...
load_opus_lib()
startup_extensions = ["chooser","interactions"]
...
class HyperBot(discord.Client):
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg += "```"
            helpmsg += "It's right there, you blindy!"
    ...
    async def cmd_pissed(self, message):
       ...
       """
       pisem = discord.Embed(title='SO FKIN PISSED!', description=' ' , color=0x7A0000)
    ...
    async def cmd_ping(self, message):
        ...
        ping = now - msgtime
        try:
            pong = discord.Embed(title='Pong! Response Time:', description='%0.01f s' % ping , color=0x7A0000)
...
#Horse Race
    async def cmd_race(self, message, user_mentions):
        """
        Usage:
            {command_prefix}race[@user1,@user2,@user3,@user4]
			
        Race between any four users.
        """
        pos1 = 0
        pos2 = 0
        pos3 = 0
        pos4 = 0
        for i in range(3):
           user1 = user_mentions[0]
           user2 = user_mentions[1]
           user3 = user_mentions[2]
           user4 = user_mentions[3]
           temp = ' '
           await self.send_message(message.channel, "\n Round %d:" %(i+1) )
           self.msg11 = await self.send_message(message.channel, "`\n %s's Horse:`" % user1.name)
           self.msg1 = self.msg11.content.strip()
           #resety1 = self.msg1.replace(':horse:','')
           #await asyncio.sleep(2)
           #await self.delete_message(self.msg11)
           #await self.send_message(message.channel, resety1)
           r11 = random.randint(20,40)
           temp = temp * (r11 + pos1)
           #print(temp+'y')	
           msgsav = await self.send_message(message.channel, "\u200B %s :horse:" % temp )
           pos1 = msgsav.content.index(':')
     #p2
           temp = ' '
           self.msg12 = await self.send_message(message.channel, "`\n %s's Horse:`" % user2.name)
           self.msg2 = self.msg12.content.strip()
           #resety2 = self.msg2.replace(':horse:','')
           #await asyncio.sleep(2)
           #await self.delete_message(self.msg12)
           #await self.send_message(message.channel, resety2)
           r12 = random.randint(20,40)
           temp = temp * (r12 + pos2)		
           #print(temp+'y')	
           msgsav = await self.send_message(message.channel, "\u200B %s :horse:" % temp )
           pos2 = msgsav.content.index(':')
    #p3
           temp = ' '
           self.msg13 = await self.send_message(message.channel, "`\n %s's Horse:`" % user3.name)
           self.msg3 = self.msg13.content.strip()
           #resety3 = self.msg3.replace(':horse:','')
           #await asyncio.sleep(2)
           #await self.delete_message(self.msg13)
           #await self.send_message(message.channel, resety3)
           r13 = random.randint(20,40)
           temp = temp * (r13 + pos3)		
           #print(temp+'y')	
           msgsav = await self.send_message(message.channel, "\u200B %s :horse:" % temp )
           pos3 = msgsav.content.index(':')
    #p4
           temp = ' '
           self.msg14 = await self.send_message(message.channel, "`\n %s's Horse:`" % user4.name)
           self.msg4 = self.msg14.content.strip()
           #resety4 = self.msg4.replace(':horse:','')
           #await asyncio.sleep(2)
           #await self.delete_message(self.msg14)
           #await self.send_message(message.channel, resety4)
           r14 = random.randint(20,40)
           temp = temp * (r11 + pos4)		
           #print(temp+'y')	
           msgsav = await self.send_message(message.channel, "\u200B %s :horse:" % temp )
           pos4 = msgsav.content.index(':')    
	
        maxy = [pos1,pos2,pos3,pos4]
        winner = max(maxy)
        print(winner)
        checker = maxy.index(winner)
        print(checker)
        await self.send_message(message.channel, "%s is the winner! :trophy: " %user_mentions[i].mention)

	#res_final	
#End Race

#profiles_and_related	 
    async def cmd_profile(self,message,author,user_mentions):
       ...
       with open('bot_files/users.json', 'r+') as f:
          datac = json.load(f)
       if datac.get(str(userc)):    
          coiny = datac[str(userc)]['coins']
          roboty = datac[str(userc)]['robotcoin']
          timey = datac[str(userc)]['timecheck']	   
       else:
          coiny = "100 :moneybag:"
          roboty = "0 :robot:"
          timey = datetime.datetime.now().strftime('%B %d,%Y %H:%M:%S')
       try:
          bioc = prof[str(userc)]['Bio']
          #print(bioc)		  
       except:		  
          bioc = "Yet to show creativity."
       try:
          marryc = prof[str(userc)]['Married To:']
          print(marryc)		  
       except:		  
          marryc = "Single."		  
       ...
       prof[str(userc)] = OrderedDict(profl)
       dumpy = json.dumps(prof)
       with open('bot_files/user_profiles.json', 'w+') as f:
          f.write(dumpy)   		  
       profem = discord.Embed(title = 'Profile info for %s' %userc, description='', color=0xff0000)
       for key in prof[str(userc)]:
          #print(key)
          #print(prof[str(userc)][key])
          profem.add_field(name=key,value=prof[str(userc)][key])
       profem.set_thumbnail(url=avatarc)
       #profem.set_image(url="http://33.media.tumblr.com/b9542e2fee4ce17ea18356f24a848bc7/tumblr_nkv1rkpJEL1tqh9pgo1_500.gif")   
       await self.send_message(message.channel,content=None,embed=profem)
    ...
    async def cmd_setbio(self,message):
       ...
       prof[str(message.author)]['Bio'] = bioc
       #print(str(message.author)+":"+bioc)
       dumpy = json.dumps(prof)
       with open('bot_files/user_profiles.json', 'w+') as f:
          f.write(dumpy) 
       return Response("Done! :thumbsup::skin-tone-1: " , reply = True, delete_after=20)
    ...
    async def cmd_marry(self,message,user_mentions):
       ...
       with open('bot_files/user_profiles.json', 'r+') as f:
          prof = json.load(f)
       if prof[str(message.author)]['Married To:'] == str(userc):
          await self.send_message(message.channel, "Hey %s, you're already married to %s !" % (message.author.mention, userc.mention))	
       elif not(prof[str(message.author)]['Married To:'] == "Single."):
          await self.send_message(message_channel, "Sorry %s, but %s is already married to %s. Ask them to get divorced first. :smiling-imp:" % (message.author.mention, userc.mention, prof[str(message.author)]['Married To:']))   
       else:
          ...
          if ("accept" in answer) or ("yes" in answer):
             ...
             prof[str(userc)]['Married To:'] = str(message.author)
             prof[str(message.author)]['Married To:'] = str(userc)
             #print(prof[str(userc)]['Married To:'])
             dumpy = json.dumps(prof)
             with open('bot_files/user_profiles.json', 'w+') as f:
                f.write(dumpy)
          else:
    ...
    async def cmd_divorce(self,message,user_mentions):
       ...
       with open('bot_files/user_profiles.json', 'r+') as f:
          prof = json.load(f)
       if not(prof[str(message.author)]['Married To:'] == str(userc)):
          await self.send_message(message_channel,"Hey %s! You're not even married to %s." % (message.author.mention, userc.mention))
       else:
          prof[str(userc)]['Married To:'] = "Single."
          prof[str(message.author)]['Married To:'] = "Single."
...
#currency
    async def cmd_daily(self, message, author):
       ...
       with open('bot_files/users.json', 'r+') as f:
          datac = json.load(f)
       if not datac.get(str(author)):
          coiny = str(100) + " :moneybag:" 
          timey = datetime.datetime.now().strftime('%H:%M:%S')
          datac[str(author)] = {'coins' : coiny, 'robotcoin' : '0 :robot:', 'timecheck': timey}
          ...
          timey2 = datac[str(author)]['timecheck']
          #print("\nexisting: "+timey2)
          ...
          if ((60 * hours + minutes) > 3600) or not (message.author.id is self.config.owner_id):
             coiny = datac[str(author)]['coins']
             roboty = datac[str(author)]['robotcoin']
             coiny_getter = coiny.split(' ')[0]
             coiny_int = int(coiny_getter)
             coiny_int = coiny_int + 100
             coiny = str(coiny_int) + " :moneybag:"
             datac[str(author)]['coins'] = coiny
             timey1 = timey + timedelta(hours=24)
             timey1 = timey1.strftime('%H:%M:%S') 
             datac[str(author)]['timecheck'] = timey1
             dumpy = json.dumps(datac)
             with open('bot_files/users.json', 'w+') as f:
                f.write(dumpy)
             ...
             if coiny_int > 1000:
                ...
                if mrep.startswith('yes') or mrep.startswith('h!yes'):
                  ...
                  datac[str(author)]['robotcoin'] = roboty
                  dumpy = json.dumps(datac)
                  with open('bot_files/users.json', 'w+') as f:
                     f.write(dumpy)   				
    ...
    async def cmd_checkbalance(self, message):
       ...
       with open('bot_files/users.json', 'r+') as f:
          datac = json.load(f)
       roboty = datac.get(str(message.author))['robotcoin']
       coiny = datac.get(str(message.author))['coins']
       coiny_getter = coiny.split(' ')[0]
       coiny_int = int(coiny_getter)	   
       ...
       if coiny_int < 1:
          ...
          await self.send_message(message.channel, "Hey %s, your current balance is: %s and %s." % (message.author.mention, coiny))			   
       ...
       if coiny_int < 1:
          ...
          if ("yes" in mrep) or ("yea" in mrep):
             ...
             if int(mrep1) == r:
                ...
                await asyncio.sleep(5)
                await self.delete_message(lem)			 
                coiny1 = datac[str(message.author)]['coins']
                ...
                if coiny_int < 1:
                   coiny_int = 1
                coiny = str(coiny_int) + " :moneybag:"
                datac[str(message.author)]['coins'] = coiny
                if not datac.get(str(user)):
                   coiny1 = amt + " :moneybag:" 
                   datac[str(user)] = {'coins' : coiny1, 'robotcoin' : '0 :robot:', 'timecheck': '24 hours'}
                else:
                   coiny1 = datac[str(user)]['coins']
                   coiny_getter = coiny1.split(' ')[0]
                   coiny_int = int(coiny_getter)
                   coiny_int = coiny_int + int(amt)
                   coiny1 = str(coiny_int) + " :moneybag:"
                   datac[str(user)]['coins'] = coiny1
                dumpy = json.dumps(datac)
                with open('bot_files/users.json', 'w+') as f:
                   f.write(dumpy)
...
#triggerwords		
        message_content = message.content.strip()
        if not message_content.startswith(self.config.command_prefix) and not message_content.lower().startswith('hey') and not message_content.lower().startswith('ayy'):
            return
        
        elif message_content.lower().startswith('hey'):
           if message.author == self.user:
              ...
              #print(str(culprit)+":1")
                 ...
                 #print(str(culprit)+":2")
                  #culprit = discord.utils.get(message.server.members, searchword.lower() in nick.lower())
            ...
            await self.add_reaction(message, "\U0001F595")
#endtw
        if message.author == self.user:
