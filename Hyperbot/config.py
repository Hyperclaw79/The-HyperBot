...
class Config:
    def __init__(self, config_file):
        ...
        confsections = {"Credentials", "Permissions", "Chat", "HyperBot"}.difference(config.sections())
        if confsections:
            raise HelpfulError(
        ...
        self.default_volume = config.getfloat('HyperBot', 'DefaultVolume', fallback=ConfigDefaults.default_volume)
        self.skips_required = config.getint('HyperBot', 'SkipsRequired', fallback=ConfigDefaults.skips_required)
        self.skip_ratio_required = config.getfloat('HyperBot', 'SkipRatio', fallback=ConfigDefaults.skip_ratio_required)
        self.save_videos = config.getboolean('HyperBot', 'SaveVideos', fallback=ConfigDefaults.save_videos)
        self.now_playing_mentions = config.getboolean('HyperBot', 'NowPlayingMentions', fallback=ConfigDefaults.now_playing_mentions)
        self.auto_summon = config.getboolean('HyperBot', 'AutoSummon', fallback=ConfigDefaults.auto_summon)
        self.auto_playlist = config.getboolean('HyperBot', 'UseAutoPlaylist', fallback=ConfigDefaults.auto_playlist)
        self.auto_pause = config.getboolean('HyperBot', 'AutoPause', fallback=ConfigDefaults.auto_pause)
        self.HiSwitch = config.getboolean('HyperBot', 'HiSwitch', fallback=ConfigDefaults.HiSwitch)
        self.delete_messages  = config.getboolean('HyperBot', 'DeleteMessages', fallback=ConfigDefaults.delete_messages)
        self.delete_invoking = config.getboolean('HyperBot', 'DeleteInvoking', fallback=ConfigDefaults.delete_invoking)
        self.debug_mode = config.getboolean('HyperBot', 'DebugMode', fallback=ConfigDefaults.debug_mode)
